...
func GetAndCreateStaticPushObject(rtmpurl string) *StaticPush {
	...
	logs.Info("GetAndCreateStaticPushObject: %s, return %v", rtmpurl, ok)
	if !ok {
		g_MapLock.RUnlock()
...
func ReleaseStaticPushObject(rtmpurl string) {
	...
	if _, ok := G_StaticPushMap[rtmpurl]; ok {
		...
		logs.Info("ReleaseStaticPushObject %s ok", rtmpurl)
		g_MapLock.Lock()
		delete(G_StaticPushMap, rtmpurl)
		g_MapLock.Unlock()
	} else {
		g_MapLock.RUnlock()
		logs.Info("ReleaseStaticPushObject: not find %s", rtmpurl)
	}
}
...
func (self *StaticPush) Start() error {
	...
	logs.Info("static publish server addr:%v starting....", self.RtmpUrl)
	err := self.connectClient.Start(self.RtmpUrl, "publish")
	if err != nil {
		logs.Info("connectClient.Start url=%v error", self.RtmpUrl)
		return err
	}
	logs.Info("static publish server addr:%v started, streamid=%d", self.RtmpUrl, self.connectClient.GetStreamId())
...
func (self *StaticPush) Stop() {
	...
	logs.Info("StaticPush Stop: %s", self.RtmpUrl)
...
func (self *StaticPush) sendPacket(p *av.Packet) {
	...
	//logs.Info("Static sendPacket: rtmpurl=%s, length=%d, streamid=%d",
...
func (self *StaticPush) HandleAvPacket() {
	if !self.IsStart() {
		logs.Info("static push %s not started", self.RtmpUrl)
		return
	}
	...
	for {
		...
		case ctrlcmd := <-self.sndctrl_chan:
			if ctrlcmd == STATIC_RELAY_STOP_CTRL {
				...
				logs.Info("Static HandleAvPacket close: publishurl=%s", self.RtmpUrl)
				break
			}
		}
	}
